...
function providersPage(p){
  const terms=[['Service Provider','服務提供者','第四方公司名稱；應核對實際服務關係。'],['Trend / Rating','評等趨勢／目前評等','觀察第四方資安評等與變化，非第三方公司的評等。'],['Industry','產業','提供者所屬產業。'],['Importance','重要程度','依第三方網域對第四方產品的依賴數量與重要性彙整。'],['Percent Dependent','依賴比例','目前範圍內依賴此提供者的第三方比例，須看分母。'],['Dependent Companies','依賴公司','開啟關聯的第三方清單，核對具體關係。'],['Certainty Level','確定程度','平台對服務關聯的證據強度。'],['Dependent Assets','依賴資產','檢查第三方哪些資產與此服務有關。'],['Associated Products','相關產品','查看此第四方提供的產品與使用公司。'],['Subscribe / Go to Company','訂閱／前往公司','是否已訂閱影響可見詳情；訂閱可能涉及配額。']];
  return `<article class="doc"><button class="back-link" data-page="fourth-overview">← 返回第四方總覽</button><div class="eyebrow">第四方風險 / Service Providers</div><h1>服務提供者</h1><p class="lead">此頁列出第三方所使用的第四方服務提供者，重點是找出共同依賴、檢查關聯證據，並決定哪些第四方值得進一步監控。不要只憑單一產品偵測推定合約或資料處理關係。</p><div class="callout"><strong>進入路徑</strong><br>頂端選擇資料夾或 Tier，再開啟 <code>4th Party Risk → Service Providers</code>。本次原頁僅顯示標題，表格未載入；下列欄位依 BitSight 官方說明整理，不臆測帳號結果。</div><h2>專有名詞與欄位</h2><div class="doc-table"><table><thead><tr><th>英文</th><th>建議中文</th><th>用途與判讀</th></tr></thead><tbody>${terms.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div><h2>建議操作流程</h2><ol><li>確認目前資料夾或 Tier；依服務提供者名稱、產業、評等及依賴比例找共同依賴。</li><li>開啟 Dependent Companies，核對第三方公司、Certainty Level 與 Dependent Assets。</li><li>查看 Associated Products，釐清實際產品與業務用途，避免將所有產品視為同一風險。</li><li>對高依賴且低評等的第四方，向第三方詢問服務範圍、替代方案及事件通知機制。</li><li>要訂閱第四方前，先確認配額與組織監控政策；可匯出 CSV 做受控追蹤。</li></ol><div class="example"><strong>使用情境：辨識供應鏈單點風險</strong><p>先按 Percent Dependent 找到被多家關鍵供應商使用的服務提供者，再打開依賴公司與資產清單確認證據。若證據強度不足，列為待供應商確認，而非已確認共同依賴。</p></div><h2>容易誤判的地方</h2><ul><li><strong>Importance 不是公司 Tier：</strong>它根據產品依賴關係與第三方網域重要性計算。</li><li><strong>關聯不等於直接契約：</strong>第四方可能經由產品或技術服務間接出現。</li><li><strong>評等與訂閱狀態影響可見資訊：</strong>沒有詳細評等不代表沒有風險。</li></ul><p class="note">依 2026-09-17 可見頁面及 BitSight 官方文件整理；中文為教學用建議譯名。</p><h2>參考資料</h2><ul><li><a href="https://help.bitsighttech.com/hc/en-us/articles/4406194178327-Service-Providers-Used-by-Third-Parties" target="_blank" rel="noopener noreferrer">BitSight：Service Providers Used by Third Parties</a></li></ul><h2>接著閱讀</h2><div class="feature-grid"><button class="feature-card" data-page="products"><strong>產品</strong><small>Products · 側欄下一個主題。</small></button><button class="feature-card" data-page="fourth-overview"><strong>第四方總覽</strong><small>Overview · 返回集中度視圖。</small></button></div></article>`;
}

function productsPage(p){
  const terms=[['Product','產品','第四方提供的具體產品名稱。'],['Service Provider','服務提供者','提供產品的第四方公司。'],['Product Types','產品類型','產品所屬技術或業務功能分類。'],['Business Function','業務功能','官方欄位用於分類服務提供者或產品用途。'],['Dependent Companies','依賴公司','目前範圍中使用該產品的第三方數量或比例，需核對分母。'],['Importance','重要程度','依使用產品的第三方網域數量與重要性計算。'],['Percent Dependent','依賴比例','可用 0–100% 範圍篩選共同產品依賴。'],['Search / Download','搜尋／下載','找特定產品或匯出 CSV；資料應受控保存。']];
  return `<article class="doc"><button class="back-link" data-page="providers">← 返回服務提供者</button><div class="eyebrow">第四方風險 / Products</div><h1>產品</h1><p class="lead">Products 頁列出第三方所使用的第四方產品，適合識別共同技術依賴及產品集中風險。產品出現在清單中是調查線索，不能單獨證明特定版本、部署方式或弱點曝險。</p><div class="callout"><strong>進入路徑</strong><br>頂端選擇資料夾或 Tier，再開啟 <code>4th Party Risk → Products</code>。本次原頁只顯示標題，表格未載入；以下欄位依 BitSight 官方文件整理，不推定帳號內結果。</div><h2>專有名詞與欄位</h2><div class="doc-table"><table><thead><tr><th>英文</th><th>建議中文</th><th>用途與判讀</th></tr></thead><tbody>${terms.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div><h2>篩選與操作</h2><ul><li><strong>Business Function、Product Type：</strong>按業務功能及產品類型縮小清單。</li><li><strong>Importance：</strong>可選 Critical、High、Medium、Low，優先檢查高重要程度產品。</li><li><strong>Percent Dependent：</strong>用依賴比例區間找出可能的集中點。</li><li><strong>Search、Download：</strong>查詢特定產品或匯出 CSV；匯出後按內部資料處理規範分享。</li></ul><h2>建議操作流程</h2><ol><li>確認目前資料夾或 Tier，避免把不同範圍的依賴比例直接比較。</li><li>依產品名稱、類型與業務功能篩選，再按 Importance 與 Percent Dependent 排序。</li><li>開啟 Dependent Companies，確認哪些第三方使用產品，以及相關網域或資產證據。</li><li>對高集中度產品，確認供應商使用版本、替代方案、漏洞通報與復原安排。</li><li>如有 CVE，回到 Vulnerability Detection 或 Findings 核對實際曝險，不以產品名稱直接推論漏洞。</li></ol><div class="example"><strong>使用情境：共用產品出現重大弱點</strong><p>搜尋產品並列出依賴的關鍵第三方，向各方確認版本與修補狀態。即使清單顯示同一產品，也可能有不同部署及安全控制。</p></div><h2>容易誤判的地方</h2><ul><li><strong>產品依賴不等於實際資料流：</strong>需確認產品用途及處理資料範圍。</li><li><strong>Importance 不是 CVSS：</strong>它反映產品與第三方網域的依賴重要性。</li><li><strong>同名產品可能版本不同：</strong>弱點判讀需具體資產與版本證據。</li></ul><p class="note">依 2026-09-17 可見頁面及 BitSight 官方文件整理；中文為教學用建議譯名。</p><h2>參考資料</h2><ul><li><a href="https://help.bitsighttech.com/hc/en-us/articles/4406193673623-4th-Party-Risk-Products" target="_blank" rel="noopener noreferrer">BitSight：4th Party Risk — Products</a></li></ul><h2>接著閱讀</h2><div class="feature-grid"><button class="feature-card" data-page="portfolio-alerts"><strong>投資組合警示</strong><small>Portfolio Alerts · 側欄下一個主題。</small></button><button class="feature-card" data-page="providers"><strong>服務提供者</strong><small>Service Providers · 核對產品來源。</small></button></div></article>`;
}

function portfolioAlertsPage(p){
  const cats=[['Informational Risk Vectors','資訊性風險面向','例如外洩憑證；資訊性項目未必影響評等。'],['NIST CSF Grades','NIST CSF 等級','框架相關等級跨越設定門檻時提醒。'],['Public Disclosures','公開揭露','可細分外洩安全事件、一般安全事件及其他揭露。'],['Risk Vector Grades','風險面向等級','風險面向等級達到或跨越門檻。'],['Security Ratings','資安評等','評等分數或百分比變化、門檻變化。'],['Vulnerabilities and Infections','弱點與感染','受監控公司出現新弱點或感染線索。'],['Threat Intelligence Events','威脅情報事件','由威脅情報觸發的警示，需核對證據。']];
  const fields=[['Increase / Decrease / Critical Decrease','上升／下降／重大下降','變化方向與程度；不是所有類別都用相同邏輯。'],['Date Range','日期範圍','調整查詢期間；零筆時先核對日期。'],['Tiers, Folders, & Subscriptions','層級、資料夾與訂閱','限制管理範圍；需與頂端公司範圍一起看。'],['Date / Company / Category / Alert','日期／公司／類別／警示內容','表格核心欄位，用來建立調查紀錄。'],['Tier / Folder / Subscription','層級／資料夾／訂閱','顯示警示所屬管理分組。'],['Alert Settings','警示設定','設定規則與接收方式，不是此頁歷史清單。']];
  const table=rows=>`<div class="doc-table"><table><thead><tr><th>英文</th><th>建議中文</th><th>功能與判讀</th></tr></thead><tbody>${rows.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
  return `<article class="doc"><button class="back-link" data-page="products">← 返回產品</button><div class="eyebrow">警示 / Portfolio Alerts</div><h1>投資組合警示</h1><p class="lead">Portfolio Alerts 是第三方投資組合的觸發紀錄，可依類別、變化程度、期間與公司範圍篩選。每筆警示仍需回到公司、發現事項或弱點詳情查證，不能把警示本身當成完整事件結論。</p><div class="callout"><strong>進入路徑與目前畫面</strong><br>左側 <code>Alerts → Portfolio Alerts</code>；右上可前往 Alert Settings。本次畫面表格顯示 0 Rows，應先檢查範圍、期間及警示規則，不能直接推論沒有風險。</div><h2>Category 警示類別</h2>${table(cats)}<h2>其他篩選與表格欄位</h2>${table(fields)}<h2>表格工具</h2><ul><li><strong>Views、Create View：</strong>保存常用的警示篩選。</li><li><strong>Search、Download as CSV、Enter full screen：</strong>查找、匯出或放大表格；沒有資料時匯出可能停用。</li><li><strong>CSV 與畫面差異：</strong>官方說明部分 Security Ratings 詳細變化只在畫面欄位，未包含於 CSV；匯出後仍要核對原頁。</li></ul><h2>建議操作流程</h2><ol><li>先確認公司範圍、Date Range、Tier／Folder／Subscription，以及是否已有 Views 條件。</li><li>依 Category 和 Severity 篩出重大下降、公開揭露、弱點及情報事件。</li><li>點開警示核對觸發時間、公司、門檻與具體證據；記錄初步判斷。</li><li>前往公司 Overview、Findings、Breach Intelligence 或 Vulnerability Detection 確認影響。</li><li>需長期監控時再到 Alert Settings 檢查規則與接收者，避免重複或漏報。</li></ol><div class="example"><strong>使用情境：每日供應商風險巡檢</strong><p>固定 7 或 30 天期間、關鍵 Tier 與重大下降條件。對每筆新警示記錄證據連結、負責人及下次複查日期；若沒有警示，仍確認設定是否啟用。</p></div><h2>容易誤判的地方</h2><ul><li><strong>0 Rows 不是沒有風險：</strong>可能是範圍、期間、訂閱或規則造成。</li><li><strong>評等變化不是已遭入侵：</strong>需核對風險面向與發現證據。</li><li><strong>資訊性風險面向不一定影響評等：</strong>但仍可能值得處理。</li></ul><p class="note">依 2026-09-17 可見介面及 BitSight 官方文件整理；中文為教學用建議譯名。</p><h2>參考資料</h2><ul><li><a href="https://help.bitsighttech.com/hc/en-us/articles/360052924933-Alert-Categories" target="_blank" rel="noopener noreferrer">BitSight：Alert Categories</a></li><li><a href="https://help.bitsighttech.com/hc/en-us/articles/29670099422487-CM-App-Downloading-Alert-Details" target="_blank" rel="noopener noreferrer">BitSight：Downloading Alert Details</a></li></ul><h2>接著閱讀</h2><div class="feature-grid"><button class="feature-card" data-page="fourth-alerts"><strong>第四方警示</strong><small>Fourth Party Alerts · 側欄下一個主題。</small></button><button class="feature-card" data-page="alert-settings"><strong>警示設定</strong><small>Alert Settings · 調整觸發規則。</small></button></div></article>`;
}

function fourthAlertsPage(p){
  const rows=[['Public Disclosures','公開揭露','目前頁面的第四方公開事件類型入口。'],['Breach Security Incident','資料外洩安全事件','可能涉及資料不當存取或外洩的事件分類。'],['General Security Incident','一般安全事件','不一定屬於資料外洩的安全事件。'],['Other Disclosure','其他揭露','其他公開揭露分類，需查看內容。'],['Date Range / Folders / Tiers','日期範圍／資料夾／層級','限制時間與第三方管理範圍。'],['Subscriptions Folder','訂閱資料夾','依訂閱分組縮小警示。'],['Date / Service Provider / Rating','日期／服務提供者／評等','辨識第四方及當前評等。'],['Category / Incident Type / Public Discovery','類別／事件類型／公開發現','釐清事件分類與公開資訊時間。'],['Folder / Companies','資料夾／公司數','查看受牽連的第三方範圍；數字需點入確認。'],['Alert Settings','警示設定','調整第四方事件提醒規則，不是警示歷史。']];
  return `<article class="doc"><button class="back-link" data-page="portfolio-alerts">← 返回投資組合警示</button><div class="eyebrow">警示 / Fourth Party Alerts</div><h1>第四方警示</h1><p class="lead">Fourth Party Alerts 將第四方服務提供者的公開事件與所選第三方範圍連結，協助判斷哪幾家供應商可能間接受影響。看到事件不等於所有依賴公司都受害，必須查明關聯與實際影響。</p><div class="callout"><strong>進入路徑與目前畫面</strong><br>左側 <code>Alerts → Fourth Party Alerts</code>。本次頁面顯示 Public Disclosures、0 Rows；要先檢查期間、資料夾、Tier、訂閱與警示設定，不能推論沒有第四方事件。</div><h2>專有名詞與表格欄位</h2><div class="doc-table"><table><thead><tr><th>英文</th><th>建議中文</th><th>用途與判讀</th></tr></thead><tbody>${rows.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div><h2>清單工具</h2><ul><li><strong>Categories：</strong>依 Breach Security Incident、General Security Incident 或 Other Disclosure 篩選。</li><li><strong>Views、Create View：</strong>儲存常用調查範圍。</li><li><strong>Search、Download as CSV、Enter full screen：</strong>搜尋、匯出或放大；沒有資料時匯出停用。</li><li><strong>Companies：</strong>查看間接受影響的第三方；同一第四方事件可能關聯多家公司。</li></ul><h2>建議操作流程</h2><ol><li>確認目前選取的資料夾或 Tier，再檢查 Date Range、Categories 與 Views。</li><li>對新警示核對第四方名稱、事件來源、公開時間、分類及評等。</li><li>點開 Companies，列出間接受影響的第三方與具體依賴服務；區分直接受害與間接關聯。</li><li>向關鍵第三方詢問實際服務使用、資料影響、備援與事件應變進度。</li><li>按內部標準記錄通報、負責人與複查日期；必要時調整 Alert Settings。</li></ol><div class="example"><strong>使用情境：共用服務商發生事件</strong><p>先辨識事件是否為已公開揭露，再查受關聯的第三方及使用產品。若只是間接依賴，應標示「可能受影響、待確認」，避免直接寫成每家公司都已外洩。</p></div><h2>容易誤判的地方</h2><ul><li><strong>第四方事件與第三方事件不同：</strong>Companies 顯示間接關聯，不必然是直接受害公司。</li><li><strong>0 Rows 受範圍與規則影響：</strong>不能當作沒有供應鏈風險。</li><li><strong>評等不是事件影響度：</strong>需另查事件證據及服務依賴。</li></ul><p class="note">依 2026-09-17 可見介面及 BitSight 官方文件整理；中文為教學用建議譯名。</p><h2>參考資料</h2><ul><li><a href="https://help.bitsighttech.com/hc/en-us/articles/360052924933-Alert-Categories" target="_blank" rel="noopener noreferrer">BitSight：Alert Categories</a></li><li><a href="https://help.bitsighttech.com/hc/en-us/articles/14144501308567-GET-4th-Party-Public-Disclosure-Affected-Companies" target="_blank" rel="noopener noreferrer">BitSight：第四方事件受關聯公司</a></li><li><a href="https://help.bitsighttech.com/hc/en-us/articles/29670099422487-CM-App-Downloading-Alert-Details" target="_blank" rel="noopener noreferrer">BitSight：Downloading Alert Details</a></li></ul><h2>接著閱讀</h2><div class="feature-grid"><button class="feature-card" data-page="reports"><strong>報表</strong><small>Reports · 側欄下一個主題。</small></button><button class="feature-card" data-page="fourth-overview"><strong>第四方總覽</strong><small>Overview · 查看共同依賴。</small></button></div></article>`;
}

function reportTermTable(rows){return `<div class="doc-table"><table><thead><tr><th>英文</th><th>建議中文</th><th>功能與判讀</th></tr></thead><tbody>${rows.map(r=>`<tr>${r.map(c=>`<td>${esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;}
function reportsPage(p){
  const screen=[['Create Custom Report','建立自訂報表','依需求建立報表；需確認欄位、期間、範圍及權限。'],['Search','搜尋','按報表名稱尋找已儲存或可用報表。'],['My Saved and Scheduled Reports','我儲存及排程的報表','快速回到常用設定與定期產出的報表。'],['Last Saved / Next Report','上次儲存／下次報表','檢查設定更新及下次預定產出；過去日期需另查排程狀態。'],['PDF / CSV / JSON','文件／表格／資料格式','不同報表支援的輸出格式不同。'],['Featured Report','精選報表','平台推薦的報表；本次畫面顯示 Company Relative Risk。'],['Company Relative Risk','公司相對風險','比較資安事件關聯與勒索軟體目標可能性；非精確預測。'],['Saved Reports / Scheduled Reports','已儲存／排程報表','前者保留設定；後者可依日、週、月送到電子郵件。']];
  const categories=[['Assessment Questionnaires','評估問卷','查看框架對照與問卷結果。'],['Comparisons','比較','Benchmarking、Company Comparison 等相對比較。'],['Findings and Infrastructure Details','發現與基礎設施細節','檢查發現、外部資產及第三／第四方事件。'],['History and Trends','歷史與趨勢','追蹤評等、感染或弱點變化。'],['Overview and Executive Reporting','總覽與主管報告','用於單家公司或整體投資組合摘要。'],['Vendor Risk Management','供應商風險管理','偏向供應商評估與風險摘要；實際可見性依授權。']];
  return `<article class="doc"><button class="back-link" data-page="fourth-alerts">← 返回第四方警示</button><div class="eyebrow">跨模組功能 / Reports</div><h1>報表</h1><p class="lead">Reports 將單家公司或投資組合的風險資料整理成可分享的報告。先選擇報表目的與資料範圍，再核對日期、欄位、輸出格式及接收對象；報表是指定時間的快照，不會自動代表最新狀態。</p><div class="callout"><strong>進入路徑</strong><br>左側主選單 <code>Reports</code>。本次畫面可見 Create Custom Report、搜尋、My Saved and Scheduled Reports 與 Featured Report；公開導覽不列出帳號中的報表名稱、排程或內容。</div><h2>目前畫面專有名詞</h2>${reportTermTable(screen)}<h2>官方報表庫分類</h2><p>下表是 BitSight 官方報表庫的類別，不表示此帳號全部可用；內容會受產品、授權與權限影響。</p>${reportTermTable(categories)}<h2>依用途選報表</h2><ul><li><strong>主管簡報：</strong>Executive、Portfolio Overview 或 Monthly Performance，重點是固定範圍和期間。</li><li><strong>供應商比較：</strong>Benchmarking、Company Comparison；比較前核對產業、資料夾與公司組合。</li><li><strong>技術調查：</strong>Findings、Infrastructure、Portfolio Vulnerabilities 或相關事件報表，回查具體證據。</li><li><strong>供應鏈：</strong>3rd／4th Party Security Incidents，可查可能的共同曝險。</li></ul><h2>建議操作流程</h2><ol><li>先決定報表要回答的問題、讀者及保密等級，再選擇單家公司或投資組合範圍。</li><li>確認資料期間、時區、資料夾、Tier 及必要欄位；若使用已儲存報表，檢查設定是否仍適用。</li><li>檢查 PDF、CSV 或 JSON 是否符合使用情境，產出前核對範圍與權限。</li><li>排程寄送時，確認收件人、頻率、下一次執行時間及附件敏感程度。</li><li>報告中註明產製日期、期間、資料來源與限制；重要判斷仍連回原系統驗證。</li></ol><div class="example"><strong>使用情境：每月第三方風險報告</strong><p>固定同一資料夾與觀察期間，彙整評等分布、重大變化、公開事件及待改善項目。下月沿用 Saved Report 前，先核對名單變動及報表設定。</p></div><h2>容易誤判的地方</h2><ul><li><strong>Last Saved 不等於資料更新時間：</strong>仍要看報表實際產製與資料期間。</li><li><strong>Next Report 日期異常不代表確實會寄送：</strong>需進入排程詳情確認狀態與收件人。</li><li><strong>不同產品的報表權限不同：</strong>官方報表庫列出的選項不一定都在本帳號顯示。</li><li><strong>匯出檔可能敏感：</strong>限制收件人與保存位置，避免把供應商資料公開。</li></ul><p class="note">依 2026-09-17 可見介面及 BitSight 官方文件整理；中文為教學用建議譯名。官方說明歷史資料最長保留至一年前的逐日資料；實際報表仍以畫面所示期間為準。</p><h2>參考資料</h2><ul><li><a href="https://help.bitsighttech.com/hc/en-us/articles/115012825287-Reports-Repository" target="_blank" rel="noopener noreferrer">BitSight：Reports Repository</a></li><li><a href="https://help.bitsighttech.com/hc/en-us/articles/360003641453-Saved-Reports" target="_blank" rel="noopener noreferrer">BitSight：Saved Reports</a></li><li><a href="https://help.bitsighttech.com/hc/en-us/articles/360005684374-Scheduled-Reports" target="_blank" rel="noopener noreferrer">BitSight：Scheduled Reports</a></li></ul><h2>接著閱讀</h2><div class="feature-grid"><button class="feature-card" data-page="invitations"><strong>存取邀請</strong><small>Access Invitations · 下一個主題。</small></button><button class="feature-card" data-page="fourth-alerts"><strong>第四方警示</strong><small>Fourth Party Alerts · 回查報表中的事件。</small></button></div></article>`;
}

function conciseGuide(p, cfg){
  const list=(items,ordered=false)=>`<${ordered?'ol':'ul'}>${items.map(x=>`<li>${esc(x)}</li>`).join('')}</${ordered?'ol':'ul'}>`;
  return `<article class="doc"><button class="back-link" data-page="${esc(cfg.back)}">← 返回上一主題</button><div class="eyebrow">${esc(p.group)} / ${esc(p.en)}</div><h1>${esc(p.zh)}</h1><p class="lead">${esc(cfg.lead)}</p><div class="callout"><strong>進入路徑與畫面範圍</strong><br>${esc(cfg.path)}</div><h2>專有名詞與介面翻譯</h2>${reportTermTable(cfg.terms)}<h2>建議操作流程</h2>${list(cfg.steps,true)}<div class="example"><strong>使用情境</strong><p>${esc(cfg.example)}</p></div><h2>容易誤判的地方</h2>${list(cfg.cautions)}<p class="note">依 2026-09-17 可見 BitSight 介面及官方文件整理；中文為教學用建議譯名。公開導覽不顯示帳號中的公司、聯絡人或設定值。</p><h2>參考資料</h2><ul>${cfg.sources.map(([label,url])=>`<li><a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(label)}</a></li>`).join('')}</ul>${cfg.next?`<h2>接著閱讀</h2><div class="feature-grid"><button class="feature-card" data-page="${esc(cfg.next)}"><strong>${esc(pages[cfg.next].zh)}</strong><small>${esc(pages[cfg.next].en)} · 側欄下一個主題。</small></button></div>`:''}</article>`;
}
function invitationsPage(p){return conciseGuide(p,{
  back:'reports',next:'access-links',lead:'Access Invitations 是供應商協作邀請的追蹤入口，可查看已送出的邀請、啟用進度與訊息。邀請代表授予對方平台存取機會，須先確認公司、收件人及分享範圍。',
  path:'Collaboration → Access Invitations。本次原頁僅載入標題，未顯示清單；以下流程依 BitSight 官方協作文件整理，不推定帳號邀請狀態。',
  terms:[['Access Invitations / Collaboration Outbox','存取邀請／協作寄件匣','追蹤組織送出的供應商邀請與進度。'],['Vendor Access','供應商存取','讓供應商在 Client/Vendor Access Program 中查看與自身相關的資料。'],['Collaboration Reason','協作原因','Informational、Remediation、Validation、Other 等邀請目的。'],['Items for Review','待審項目','可附上相關發現或風險面向作為討論脈絡。'],['Recipient / CC','收件人／副本','核對供應商網域與副本對象，避免錯寄敏感資訊。'],['Status / View Progress','狀態／查看進度','檢查邀請是否接受、到期或仍待回應。'],['Messages','訊息','與供應商溝通及留存追蹤紀錄。']],
  steps:['先從正確供應商的 Overview 或 Findings 確定協作目的及要分享的項目。','核對收件人的公司與電子郵件網域；依最小必要原則選擇 Items for Review。','送出後在 Access Invitations 檢查狀態、訊息及到期日；邀請連結通常在 14 天後失效。','供應商啟用後透過訊息追蹤修復、驗證與下次複查日期。'],
  example:'針對重大發現邀請供應商協作：選擇 Remediation 原因、附上相關發現，確認收件人後送出，再從寄件匣追蹤是否接受及回覆。',
  cautions:['邀請過期不等於供應商拒絕；應核對信件是否送達及聯絡資訊。','CC 收件人可能收到說明信，但不一定獲得平台啟用連結。','平台協作不取代內部工單、合約通報或事件應變紀錄。'],
  sources:[['BitSight：Invite Third Parties to Collaborate','https://help.bitsighttech.com/hc/en-us/articles/360054277174-Invite-Third-Parties-to-Collaborate-in-the-Bitsight-Platform'],['BitSight：Permissions — Collaboration','https://help.bitsighttech.com/hc/en-us/articles/28055962253079-Permissions-Collaboration']]
});}

function accessLinksPage(p){return conciseGuide(p,{
  back:'invitations',next:'contacts',lead:'Vendor Access Links 可產生供應商自行申請 BitSight 存取權的專用連結。連結應只提供給目前或潛在供應商，不可放上公開網站；申請與核准仍要依組織流程控管。',
  path:'Collaboration → Vendor Access Links。頁首可見 Create Link，表格包含 Name、Description、Vendor Access Link、User、Date Created；本次畫面為 0 Links，未建立任何連結。',
  terms:[['Create Link / Generate New Token','建立連結／產生新權杖','開啟建立視窗；按 Generate 才真正產生 URL。'],['Name / Description','名稱／說明','記錄連結用途及適用供應商群。'],['Vendor Access Link','供應商存取連結','可供供應商提出申請的 URL；需視為受控資訊。'],['User / Date Created','建立者／建立日期','追查誰建立、何時建立。'],['Vendor requested access token','供應商申請存取權杖','以該權杖識別從連結進來的申請。'],['Search / 0 Links','搜尋／連結數','查找既有連結；零筆只代表目前沒有清單記錄。']],
  steps:['確認是否有建立連結權限與業務需求，為不同用途設定可辨識的 Name、Description。','閱讀建立視窗的公開分享限制，確認發送管道只面向目前或潛在供應商。','產生後記錄建立者、建立日期及內部管理者；不要將實際 URL 放進公開文件或網站。','收到申請後到 Access Invitations 確認供應商身份、網域與核准狀態。','定期盤點連結用途及仍需有效的對象；若需停用，先核對影響範圍。'],
  example:'採購流程需要讓新供應商自行申請時，建立專用連結，透過受控郵件或供應商入口發送；收到申請後再依核准流程確認身份。',
  cautions:['建立視窗明確要求不要把 URL 發布在公開網站；本中文導覽絕不顯示或儲存真實連結。','有連結不等於已核准存取；申請仍須審核。','不要在多個無關專案混用連結，否則後續難以追蹤來源。'],
  sources:[['BitSight：Continuous Monitoring Application','https://help.bitsighttech.com/hc/en-us/articles/4410655245079-Continuous-Monitoring-Application'],['BitSight：Permissions — Collaboration','https://help.bitsighttech.com/hc/en-us/articles/28055962253079-Permissions-Collaboration']]
});}

function contactsPage(p){return conciseGuide(p,{
  back:'access-links',next:'alert-settings',lead:'Vendor Contacts 用來維護供應商的業務或資安聯絡資料；Collaboration Contacts 則顯示供應商在協作計畫中指定的窗口。兩者用途不同，聯絡人資料應定期驗證並受控保存。',
  path:'Collaboration → Vendor Contacts。頁面有 Vendor Contacts 與 Collaboration Contacts 兩個頁籤；本次兩頁籤皆為 0 Contacts，未輸入或上傳任何個資。',
  terms:[['Vendor Contacts','供應商聯絡人','組織維護的供應商聯絡清單。'],['Collaboration Contacts','協作聯絡人','供應商指定的協作窗口，可能不公開姓名或電子郵件。'],['Company / Name / Email','公司／姓名／電子郵件','手動新增聯絡人的必填欄位。'],['Date Added','新增日期','用於追蹤資料多久未驗證。'],['Add Vendor Contact','新增供應商聯絡人','開啟表單；完成公司、姓名與郵件後才可新增。'],['Upload CSV','上傳 CSV','批次匯入聯絡人；應先核對格式、授權及重複資料。'],['Company / Folder / Tier','公司／資料夾／層級','依管理範圍查找聯絡人。'],['Search','搜尋','依目前頁籤查找清單。']],
  steps:['確認要維護的是 Vendor Contacts 還是檢視供應商指定的 Collaboration Contacts。','新增前向供應商確認姓名、職責、公司網域及聯絡用途，並核對個資處理依據。','手動新增時選正確公司，填寫 Name、Email；批次上傳前先清理 CSV 的重複與過期資料。','用 Company、Folder、Tier 篩選，定期複核離職、職務異動與無效信箱。','送協作邀請或事件通知前，再次確認收件者是否為適當窗口。'],
  example:'關鍵供應商發生事件時，先查 Vendor Contacts 的資安窗口，再確認 Collaboration Contacts 是否另有平台指定接收者，避免通知送到離職人員。',
  cautions:['Vendor Contacts 不一定就是平台協作接收者，兩個頁籤不可混為一談。','0 Contacts 不代表供應商沒有聯絡窗口，可能只是尚未在此系統建檔。','CSV 或聯絡人清單含個資，不應放到公開導覽或任意分享。'],
  sources:[['BitSight：Permissions — Collaboration','https://help.bitsighttech.com/hc/en-us/articles/28055962253079-Permissions-Collaboration'],['BitSight：Getting Started with Third Party Collaboration','https://help.bitsighttech.com/hc/en-us/articles/360042230653-Getting-Started-with-Third-Party-Collaboration']]
});}

function alertSettingsPage(p){return conciseGuide(p,{
  back:'contacts',next:'folders',lead:'Alert Settings 用來決定哪些公司範圍、哪些風險變化會產生警示，以及使用者是否收到郵件。設定頁與 Portfolio Alerts／Fourth Party Alerts 的歷史清單不同。',
  path:'Portfolio Settings → Alert Settings。分為 Tiers and Folders 與 Alert Sets 兩頁籤；本次兩者均顯示 0 Rows，未變更任何規則。',
  terms:[['Tiers and Folders','層級與資料夾','檢查各管理範圍套用的警示設定。'],['Alert Sets','警示集合','將警示類別與門檻組成可重用的規則。'],['Create Alert Set','建立警示集合','新增規則入口，操作前先確定範圍與權限。'],['Name / Description','名稱／說明','標示警示集合的用途。'],['Alert Set / Policy','警示集合／政策','查看各層級或資料夾套用的規則。'],['Email','電子郵件','目前使用者是否收到該範圍的警示郵件。'],['Created by / Shared','建立者／是否共用','確認規則所有權與共用範圍。'],['Actions','操作','可能提供編輯或管理，應先核對影響對象。']],
  steps:['先決定哪些 Tier、Folder 或訂閱需要監控，以及業務可接受的門檻與接收頻率。','在 Alert Sets 檢查或建立規則，明確命名並記錄類別、門檻與負責人。','回到 Tiers and Folders 確認規則套用到正確範圍、Policy 及 Email 狀態。','到 Portfolio Alerts 或 Fourth Party Alerts 驗證後續是否出現預期警示；注意產生可能有資料處理延遲。','定期清理重複規則和過期收件人，並保留設定變更紀錄。'],
  example:'對關鍵 Tier 設較嚴格的評等下降與重大事件警示，對較低關鍵性的供應商使用不同門檻；上線後觀察警示量與漏報情況再調整。',
  cautions:['0 Rows 代表目前畫面沒有規則記錄，不代表監控公司沒有風險。','個人 Email 設定與組織共用 Alert Set 是不同層級，不能互相代替。','修改共用規則可能影響多位使用者；先確認權限與受影響範圍。'],
  sources:[['BitSight：Alert Categories','https://help.bitsighttech.com/hc/en-us/articles/360052924933-Alert-Categories'],['BitSight：Permissions — Alerts, Notifications & Email','https://help.bitsighttech.com/hc/en-us/articles/28059398480023-Permissions-User-Settings-Alerts-Notifications-Email']]
});}

function foldersPage(p){return conciseGuide(p,{
  back:'alert-settings',next:'tiers',lead:'Folder Management 依業務、供應商類型或管理目的分組公司，方便篩選、比較及設定警示。資料夾是管理視圖，不是公司評等，也不等於 Tier。',
  path:'Portfolio Settings → Folder Management。本次查看時原系統工作階段逾時，無法核對帳號內清單；以下為 BitSight 官方文件所列功能，未操作真實資料夾。',
  terms:[['Create a folder','建立資料夾','設定 Folder Name，可選填 Folder Description。'],['Folder Name / Description','資料夾名稱／說明','標示用途與收錄標準，方便日後管理。'],['# of Companies','公司數','此資料夾目前包含的公司數。'],['Average Rating / Median Rating','平均／中位評等','依畫面與版本標籤判讀群組評等，勿混用兩者。'],['Owner','擁有者','有權管理資料夾設定的人員。'],['Share Folder / Enable Sharing','分享資料夾／啟用分享','指定可見對象及 View、Manage Companies、Own 等權限。'],['Download / Filters / Search','下載／篩選／搜尋','匯出 CSV、依公司數或擁有者縮小清單。'],['Remove from Folder / Delete','移出資料夾／刪除','兩者影響不同，動作前應確認目標與權限。']],
  steps:['先定義分組目的與收錄標準，例如關鍵供應商、IT 服務或業務區域。','建立資料夾時填明確名稱與說明，避免和 Tier 或公司關係混淆。','從 Companies List 加入公司，再核對公司數及群組評等。','若需分享，選擇 All Users、Specific Users 或 Access Control Groups，給予最小必要權限。','定期檢查公司異動與警示規則；刪除資料夾前先確認相關報表、檢視和權限。'],
  example:'建立「關鍵支付供應商」資料夾，納入實際承接支付服務的公司，設定專用警示並僅分享給採購與資安管理者。',
  cautions:['資料夾目前不能巢狀建立；不要把它設計成多層組織樹。','移出資料夾不等於取消公司訂閱；刪除資料夾也不是刪除公司。','資料夾分享會改變其他使用者可見範圍，須審核權限。'],
  sources:[['BitSight：Folder Management','https://help.bitsighttech.com/hc/en-us/articles/360039423453-Folder-Management'],['BitSight：Folders','https://help.bitsighttech.com/hc/en-us/articles/231700807-Folders']]
});}

function tiersPage(p){return conciseGuide(p,{
  back:'folders',next:'integrations',lead:'Tier Settings 依供應商對組織的重要性分層，並為每層設定評等風險門檻、評估與警示。Tier 是內部治理分類，不是 BitSight Security Rating 的高低分組。',
  path:'Portfolio Settings → Tier Settings。本次原系統工作階段已逾時，無法核對帳號實際 Tier；以下依 BitSight 官方文件整理，未修改設定。',
  terms:[['Tier / Create Tier','層級／建立層級','最多可設定 5 個自訂層級並調整順序。'],['Global Tiers / Group Tiers','全域／群組層級','適用所有使用者，或限定 Access Control Group。'],['Tier Details','層級詳情','名稱、說明及收錄的公司；一家公司同時只能屬於一個 Tier。'],['Risk Thresholds','風險門檻','設定各 Tier 的資安評等門檻，影響風險矩陣與行動方案。'],['Assessment','評估','可把已上傳的評估自動關聯至此 Tier 公司。'],['Configure Alerts','設定警示','讓不同重要程度的供應商使用不同觸發規則。'],['Tier Recommender','層級建議器','提供建議供人工審查，非自動確定業務關鍵性。'],['Edit Tier / Delete Tier','編輯／刪除層級','會影響分類、報表與警示，操作前須評估。']],
  steps:['先與採購、業務和資安部門定義各 Tier 的資料敏感度、網路連線、營運依賴與中斷容忍度。','建立最多 5 層，為每層填名稱、說明與收錄標準，並按關鍵性排序。','從 Companies List 或 Vendor Overview 指派公司；確認同一家公司只屬於一個 Tier。','設定 Risk Thresholds、Assessment 與 Alerts，再到 Portfolio Risk Matrix 檢查結果。','定期複核供應商業務變化與 Tier Recommender 建議；調整前預估對報表和警示的影響。'],
  example:'將處理受管制個資且營運中斷會造成重大影響的供應商列為 Tier 1，設定較嚴格評等門檻與警示；一般低依賴服務則使用較低層級。',
  cautions:['Tier 衡量內部關鍵性，不是公司資安評等；高分公司仍可能是 Tier 1。','調整門檻可能改變風險矩陣分類與警示量，需先告知利害關係人。','Tier Recommender 是輔助建議，不能取代合約與資料流程判斷。'],
  sources:[['BitSight：Create Tiers on the Tier Settings Page','https://help.bitsighttech.com/hc/en-us/articles/360045350573-Create-Tiers-on-the-Tier-Settings-Page'],['BitSight：Tiers','https://help.bitsighttech.com/hc/en-us/articles/360008158393-Tiers']]
});}

const integrationCatalog=[
  ['Anecdotes','合規證據收集','整合合規證據與文件。'],['Aravo','第三方風險管理','在 Aravo 使用 BitSight 風險資料。'],['Archer','風險治理','在 Archer 工作流程監控第三方風險。'],['AuditBoard','GRC／供應商風險','加入評等與風險面向資料。'],['Diligent','治理與 ESG','連結組織治理、風險與合規資訊。'],['Fortress','供應鏈風險','結合供應鏈風險管理。'],['GEP','採購 S2P','把評等帶進採購流程。'],['Graphite Connect','供應商管理','整合供應商資料。'],['Microsoft Power BI','報表／儀表板','以 BitSight 資料建立自訂分析。'],['Moody’s Analytics','風險資料','結合 Orbis 等企業風險資訊。'],['Netskope','雲端風險交換','在 Netskope 使用公司與評等資料。'],['OneTrust','第三方風險管理','整合供應商風險流程。'],['Onspring','GRC','匯入評等、風險面向及警示。'],['Prevalent','供應商評估','結合評估與業務資料。'],['ProcessUnity','供應商上線','支援上線及持續改善流程。'],['Reco','身分／資料／應用風險','把 BitSight 資料帶入 Reco。'],['S&P Global','企業風險情報','監控供應商資安表現。'],['SAP Business Network','供應商網路','集中合規證據與供應商流程。'],['ServiceNow','服務／風險管理','串接供應商上線與持續監控。'],['Tableau','分析儀表板','視覺化 BitSight 資料。'],['ThreatQ','威脅情報','將資料映射為情報事件與物件。'],['Venminder','第三方管理','輔助供應商導入及風險管理。'],['Whistic','供應商評估','結合供應商風險與評估資料。'],['ZenGRC','GRC','在風險合規流程使用 BitSight 資料。'],['Coupa','即將推出','頁面標示 Coming Soon；不能視為已可設定。']
];
function integrationsPage(p){
  const controls=[['Learn more','深入了解','開啟供應商或官方說明，不代表整合已啟用。'],['Quick view / Detailed view','快速檢視／詳細檢視','查看用途、開發者、標籤和設定概述。'],['Tags','標籤','例如 Vendor Risk、Risk Ratings、GRC、Reporting、Dashboard、Assessments。'],['Install/Setup Integration','安裝／設定整合','可能依授權或頁面狀態停用；先核對所需權限。'],['Coming Soon','即將推出','尚不能視為可用整合。'],['Missing an integration? / Contact us','缺少整合？／聯絡我們','提出整合需求，不會自動新增功能。']];
  return `<article class="doc"><button class="back-link" data-page="tiers">← 返回層級設定</button><div class="eyebrow">協作與設定 / Integrations</div><h1>整合</h1><p class="lead">Integrations 是 BitSight 與 GRC、採購、分析及資安工具的整合目錄。先確認業務目的、資料流向與授權，再看個別整合的安裝要求；目錄中出現產品不表示帳號已安裝。</p><div class="callout"><strong>進入路徑</strong><br>左側 <code>Integrations</code>。本次可見目錄及 Microsoft Power BI 的詳細頁；該頁 Install/Setup Integration 按鈕停用，沒有產生 API token 或連接外部系統。</div><h2>畫面操作與狀態</h2>${reportTermTable(controls)}<h2>目前目錄中的整合項目</h2><p>以下依可見畫面逐項整理；實際支援、費用與設定步驟應開啟各項詳情再核對。</p>${reportTermTable(integrationCatalog)}<h2>設定前檢查</h2><ol><li>確認資料用途：報表、供應商風險、工單或威脅情報；選能解決工作流程問題的整合。</li><li>查看 Detailed view 的產品適用範圍、開發者、資料欄位、權限及安裝步驟。</li><li>若需 API token，先確認最小權限、保管位置、輪替週期與撤銷方式；不可貼到公開文件。</li><li>核對第三方處理資料的合約、跨境傳輸及額外授權費用，再由管理員在測試環境驗證。</li><li>啟用後抽查評等、發現、警示及更新頻率，避免資料同步延遲造成誤判。</li></ol><div class="example"><strong>使用情境：建立主管風險儀表板</strong><p>在 Power BI 詳情頁先確認所需資料與產品適用範圍，再由管理員申請適當權限的 API token，於受控環境連接並驗證資料日期。頁面示例提及 Security Posture Management，不可直接假設 Continuous Monitoring 全部欄位相同。</p></div><h2>容易誤判的地方</h2><ul><li><strong>目錄展示不是已啟用：</strong>仍需安裝、設定、授權及驗證。</li><li><strong>詳細頁可能跨產品：</strong>Power BI 示例偏向 Security Posture Management；需核對 CM 支援資料。</li><li><strong>外部連結內容會變：</strong>以目前服務商文件及合約為準。</li><li><strong>Coming Soon 不等於可用：</strong>不要規劃成已上線流程。</li></ul><p class="note">依 2026-09-17 可見 BitSight 介面與官方整合目錄整理；中文為教學用建議譯名。未安裝任何整合，未讀取或產生憑證。</p><h2>參考資料</h2><ul><li><a href="https://help.bitsighttech.com/hc/en-us/categories/204239408-Integrations-Partners" target="_blank" rel="noopener noreferrer">BitSight：Integrations & Partners</a></li><li><a href="https://help.bitsighttech.com/hc/en-us/articles/4411966701975-Bitsight-Archer-Integrations" target="_blank" rel="noopener noreferrer">BitSight：Archer Integrations</a></li></ul></article>`;
}
...
function page(p){
  ...
  if(p.id==='providers')return providersPage(p);
  if(p.id==='products')return productsPage(p);
  if(p.id==='portfolio-alerts')return portfolioAlertsPage(p);
  if(p.id==='fourth-alerts')return fourthAlertsPage(p);
  if(p.id==='reports')return reportsPage(p);
  if(p.id==='invitations')return invitationsPage(p);
  if(p.id==='access-links')return accessLinksPage(p);
  if(p.id==='contacts')return contactsPage(p);
  if(p.id==='alert-settings')return alertSettingsPage(p);
  if(p.id==='folders')return foldersPage(p);
  if(p.id==='tiers')return tiersPage(p);
  if(p.id==='integrations')return integrationsPage(p);
